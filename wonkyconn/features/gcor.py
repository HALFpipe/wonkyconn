...
# AFNI's `gcor2` computes GCOR as ||(1/M) Σ u_i||^2 for unit-variance time series,
...
def compute_gcor(matrix: npt.NDArray[np.float64]) -> float:
    ...
    # relmat already stores R, so averaging equals ||(1/M) Σ u_i||^2 from AFNI.
...
def calculate_gcor(
    ...
    # Load ROI correlation matrix and compute GCOR per subject
    data = np.fromiter(
        (
