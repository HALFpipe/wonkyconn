...
def workflow(args: argparse.Namespace) -> None:
    ...
    # Load atlases (support single pair or list of pairs)
    atlas_entries = args.atlas
    if atlas_entries and isinstance(atlas_entries[0], str):
        atlas_entries = [atlas_entries]
    atlases: dict[str, Atlas] = {
        name: Atlas.create(name, Path(atlas_path_str)) for name, atlas_path_str in atlas_entries
    }
    ...
    timeseries_entries: dict[tuple[tuple[str, str], ...], Path] = dict()
    for extension in (".tsv", ".tsv.gz", ".json"):
        for candidate in sorted(index.get(suffix="timeseries", extension=extension), key=str):
            key = tuple(sorted((k, v) for k, v in index.get_tags(candidate).items() if k != "extension"))
            timeseries_entries.setdefault(key, candidate)

    for timeseries_path in sorted(timeseries_entries.values(), key=str):
        ...
        if "NumberOfVolumes" not in metadata:
            ...
            if timeseries_path.suffix in {".tsv", ".tsv.gz"}:
                candidate_paths.append(timeseries_path)
            candidate_paths.extend(sorted(index.get_associated_paths(timeseries_path, extension=".tsv"), key=str))
            candidate_paths.extend(sorted(index.get_associated_paths(timeseries_path, extension=".tsv.gz"), key=str))

            for candidate_path in candidate_paths:
                if candidate_path.suffix == ".tsv":
                    with candidate_path.open("r") as file_handle:
                        line_count = sum(1 for _ in file_handle)
                elif candidate_path.suffixes[-2:] == [".tsv", ".gz"]:
                    with gzip.open(candidate_path, "rt") as file_handle:
                        line_count = sum(1 for _ in file_handle)
                else:
                    continue
                ...
                raise ValueError(
                    f"Unable to determine NumberOfVolumes for timeseries {timeseries_path}"
                )
        ...
        for relmat_path in index.get(**relmat_query):
            ...
            if seg not in atlases:
                gc_log.warning(
                    "Skipping connectivity matrix %s because atlas '%s' was not provided",
                    relmat_path,
                    seg,
                )
                continue
    ...
    for key, connectivity_matrices in tqdm(grouped_connectivity_matrix.items(), unit="groups"):
        if not connectivity_matrices:
            gc_log.debug(f"Skipping empty group {key}")
            continue
